defmodule Clickr.GradesTest do
  ...
  describe "bonus_grades" do
    ...
    test "update_bonus_grade/2 with valid data updates the bonus_grade" do
      bonus_grade = bonus_grade_fixture()
      update_attrs = %{name: "some updated name", percent: 0.42}

      assert {:ok, %BonusGrade{} = bonus_grade} =
               Grades.update_bonus_grade(bonus_grade, update_attrs)

      assert bonus_grade.name == "some updated name"
      assert bonus_grade.percent == 0.42
    end

    test "update_bonus_grade/2 with invalid data returns error changeset" do
      bonus_grade = bonus_grade_fixture()

      assert {:error, %Ecto.Changeset{}} = Grades.update_bonus_grade(bonus_grade, @invalid_attrs)

      assert bonus_grade == Grades.get_bonus_grade!(bonus_grade.id)
    end
